...
for i in ["blackscholes", "bodytrack", "canneal" ,"fluidanimate","freqmine", "rtview", "swaptions" ,"streamcluster"]:
	...
	for j in list1:
		...
		ref_size = 512
		ref_assoc = 8
		size_factor = 1.15 ** np.log2(int(L2_size) / ref_size)
		assoc_factor = 1.1 ** np.log2(int(L2_assoc) / ref_assoc)
...
plt.savefig("var_geometric_mean_L2", bbox_inches="tight")
...
lgd = plt.legend(lns, labs)	
lgd.draw_frame(False)
plt.savefig("var_harmonic_mean_L2", bbox_inches="tight")
